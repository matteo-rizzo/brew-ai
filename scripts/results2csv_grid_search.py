...
AGGREGATED_CSV_PATH = 'logs/aggregated_results_grid_search.csv'
...
def find_metrics_files(base_dir: str) -> pd.DataFrame:
    ...
    console.print("[bold cyan]Searching for _cv_results.json files in evaluation results...[/bold cyan]")
    ...
    for root, dirs, files in track(os.walk(base_dir), description="Processing directories..."):
        for file in files:
            if file.endswith('_cv_results.json'):
